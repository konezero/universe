"""Host-invoked automatic Checkpoint and Resume persistence coordinator.

The Runtime owns the durable store transaction. Hosts own event detection and
the bounded, redacted recovery context supplied for each trigger.
"""
...
import os
import sqlite3
import subprocess  # nosec B404 - explicit argv and shell=False below.
import tempfile
import threading
...
AUTO_CONTINUITY_SCHEMA = "ai-career.auto-continuity.v1"
...
PERSISTENT_MODE_SESSION = "PERSISTENT_MODE_SESSION"
EPHEMERAL_TASK_FRAME_SESSION = "EPHEMERAL_TASK_FRAME_SESSION"
DEFAULT_MAX_OUTPUT_CHARS = 200_000


@dataclass(frozen=True)
class NativeCliRequest:
    """Exact-argv Runtime subprocess request for one installed continuity CLI call."""
...
def _bounded(value: bytes, maximum: int) -> str:
    return value.decode("utf-8", errors="replace")[:maximum]


def run_native_cli(request: NativeCliRequest) -> NativeCliResult:
    """Run one installed Runtime CLI with explicit argv and no shell boundary."""
    ...
    if not executable.is_file():
        return NativeCliResult(
            ...
            return_code=None,
            stdout="",
            stderr="continuity Python executable is unavailable",
        )
    if executable.suffix.lower() in {".bat", ".cmd", ".ps1"}:
        return NativeCliResult(
            ...
            return_code=None,
            stdout="",
            stderr="continuity Python executable must be a native executable",
        )
    if request.cwd is not None and not request.cwd.expanduser().resolve().is_dir():
        return NativeCliResult(
            status="LAUNCH_FAILED",
            return_code=None,
            stdout="",
            stderr="continuity Runtime working directory is unavailable",
        )
    if request.timeout_seconds <= 0 or request.max_output_chars < 1:
        return NativeCliResult(
            status="LAUNCH_FAILED",
            return_code=None,
            stdout="",
            stderr="continuity Runtime native CLI request is invalid",
        )
    command = [str(executable), *request.arguments]
    try:
        completed = subprocess.run(
            command,
            cwd=str(request.cwd.resolve()) if request.cwd is not None else None,
    ...
    return NativeCliResult(
        ...
        stdout=_bounded(completed.stdout, request.max_output_chars),
        stderr=_bounded(completed.stderr, request.max_output_chars),
    )
...
class ModeContinuityLifecycle:
    """Apply one automatic continuity route to every Host mode session."""
    ...
    def __init__(
        ...
        *,
        ...
        coordinate_provider: Callable[[], Mapping[str, Any]] | None = None,
    ) -> None:
        self.coordinator = coordinator
        self.project_root = project_root
        self.mode = _required_text(mode, "mode").upper()
        self.session_kind = _required_text(session_kind, "session_kind")
        if self.session_kind not in {
            PERSISTENT_MODE_SESSION,
            EPHEMERAL_TASK_FRAME_SESSION,
        }:
            raise ContinuityCoordinatorError(
                "AUTO_CONTINUITY_SESSION_KIND_INVALID",
                f"unsupported session kind: {self.session_kind}",
            )
    ...
    def flush(
        ...
        *,
        ...
        if self.session_kind == EPHEMERAL_TASK_FRAME_SESSION:
            return {
                ...
                "reason": "EPHEMERAL_MODE_SESSION",
                "git_publication": "NOT_PERFORMED",
            }
        context = self.context_provider()
        coordinate = (
            self.coordinate_provider() if self.coordinate_provider is not None else None
        )
        return self.coordinator.save(
...
class RuntimeContinuityBackend:
    ...
    def save(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="ai-career-continuity-") as directory:
