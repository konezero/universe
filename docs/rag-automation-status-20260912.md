...
## 원본 복구·과거 기록·긴 메시지 수집

UNKNOWN91개 실제 점검에서63개 복구, 과거 활동926개 추가. 자동 배치 전 원본8개 순환 점검과 실시간 커서를 분리한 과거 수집을 연결했고,32,000자 초과 단일 활동 내부 재개를 추가했다. 관련57건 PASS. 상세 경로·운영 검증·남은28개 사유는 [수집 누락 보완](rag-collection-gaps-20260912.md)을 참조한다. 전체 원본 순환이나 의미 품질 검증 완료를 뜻하지 않는다.


## 모델 출처 참조 보완

[근거 번호 계약 v2](rag-reference-contract-20260912.md)를 적용했다. 모델은 이번 호출의 E번호를 선택하고 서버가 정확한 활동 해시로 변환한다. 잘못된 참조는 배치 전체 저장을 계속 거부하며, 근거 없는 후보를 억지로 만들지 않도록 빈 결과도 처리한다. 관련29건 PASS. 기존 출처 오류 run b26a10b9의 attempt2가166초/후보16개로 성공했고 성공 커서 갱신을 확인했다. 내용의 의미 검증과 오류율 통계는 별도다.
