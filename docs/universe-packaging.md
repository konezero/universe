...
UniversePortable-*/
  ...
  project-integration-catalog.json  # verified attached-project template catalog
...
project-integration-catalog.json is generated by loading the copied
templates/project-integration/ files from the portable folder itself.
VERSION.txt records its schema, status, path, and digest. This gives the
portable release a self-contained, digest-bound source for attached-project
binding and local Workspace guidance; it does not read Career Git at install
time.
