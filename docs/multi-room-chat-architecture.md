...
15. `POST /v1/goals/{goal_id}/automation/scheduler` is an explicit USER `START`/`PAUSE` control for the same bounded Conductor loop. Its SQLite record owns the expected Goal revision, enabled state, lease owner/expiry, tick count, last surface, and visible stop/error reason. A tick claims one expiring lease and may invoke only the existing projection/`ADVANCE` route; it stops and disables itself at USER Work Plan application, WAIT, ambiguous proposals, missing Task Frame input, Todo selection, Task Frame run, or result-application boundaries. Repeated `START` resumes from durable state, and a new service process can reclaim a due `RUNNING` record only after the old lease expires. The scheduler never runs a Task Frame, selects Todos, applies results, creates authority, or invents execution assignment.
