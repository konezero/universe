...
상태: 초안, Codex Master 공유용. 새 taxonomy 저장소나 Collector를 만들지 않는다 —
`docs/memory-candidate-type-ui-audit-20260911.md`가 확인한 기존 세 축(후보 종류 /
메모 성격 / 연결 상태)을 그대로 쓴다. 담당 A(백엔드/공통 계약, 이 문서 소유)와
Codex Master(UI, `tools/universe_ui/app.js`)가 이 계약만으로 통합한다.
...
- `tools/universe_app/feature_node_proposal.py:_source_entries` — MEMORY kind는
  ...
  분기). MEMORY+EXPLORE는 지식으로 보관될 뿐 제안 생성기 입력이 되지 않는다 —
  이건 버그가 아니라 의도된 분기이지만, 서버(`attach_memory_review_next_work`)와
  UI 둘 다 이 차이를 사용자에게 알려주지 않는다.
- `attach_memory_review_next_work`(`tools/universe_server.py:19272`)는 state가
  ...
  호출한다. MEMORY+EXPLORE에서도 호출은 일어나지만 입력에 기여하지 않으므로,
  "무언가 처리됐다"는 toast만 뜨고 실제로는 아무 자동화도 없다.
...
"decision_contract": {
  ...
  "allowed_actions": ["IGNORE", "KEEP", "EXPLORE", "START_PRODUCT_DESIGN"],
  // state != REVIEW_REQUIRED면 기본적으로 [] (재검토 가능성은 reopen에서 별도 표현)
  "disabled_actions": {
    // action -> 코드. UI는 이 코드로 문구를 결정, 서버가 의미를 소유.
    // 예: MEMORY 후보에서 EXPLORE는 막지 않되(지식 보관은 유효한 선택), 결과가
    // 제품 제안으로 이어지지 않는다는 걸 next_action으로 드러낸다(막을 필요 없음).
  },
  "current_decision": null,               // state==REVIEW_REQUIRED면 null, 아니면 state
  "next_action": {
    "kind": "NONE",
    // REVIEW_REQUIRED  -> NONE (아직 결정 안 됨)
    // MEMORY+EXPLORE   -> "KNOWLEDGE_RETAINED_NO_AUTOMATION" (자동 제안 생성 안 함)
  ...
  "reopen": {
    "allowed": false,
    "reason": null,          // ALREADY_REVIEW_REQUIRED | NOT_YET_SUPPORTED | ...
    "candidate_digest": "<sha256>"   // 재검토 API 호출 시 되돌려 보낼 값 (§2)
  }
}
```

`next_action.kind` 값 집합은 닫힌 enum
(`NONE|KNOWLEDGE_RETAINED_NO_AUTOMATION|PRODUCT_PROPOSAL_ATTEMPTED|RAG_ADOPT_AVAILABLE|ACKNOWLEDGED_NO_AUTOMATION`)으로
서버가 소유. `attach_memory_review_next_work`의 반환값(`generated.effects`)을 그대로
...
- `expected_candidate_digest`가 현재 `candidate_digest`와 다르면
  `MEMORY_CANDIDATE_DIGEST_STALE`(409) — 동시 재검토/경쟁 방지.
- 성공하면 `state`를 `REVIEW_REQUIRED`로 되돌리되, 기존 `memory_candidate_review`
  행은 **삭제하지 않는다** — 새 `memory_candidate_reopen` 테이블(또는 기존 review
  테이블에 `superseded_by_reopen_at` 컬럼)에 남겨 불변 이력을 유지한다. 재검토 후
  다시 결정하면 새 review row가 추가되고, candidate에는 review 이력 배열
  (`review_history: [...]`, 최신이 마지막)을 노출해 UI가 과거 결정을 보여줄 수
  있게 한다.
...
| 공동 완료 기준 | 이 계약의 대응 |
| --- | --- |
| kind×decision/API 직접 호출 불일치 차단 | §1 `decision_contract.allowed_actions`/`disabled_actions`, 서버가 review 요청 시에도 같은 표를 검증 |
| 명시적 재검토 digest/revision·동시성·불변 이력 | §2 `/reopen` + digest 충돌 + review_history |
