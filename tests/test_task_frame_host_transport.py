...
class TransportTests(unittest.TestCase):
    ...
    def test_master_done_exit_wakes_master_but_unsolicited_exits_remain_notes(self):
        bus = HttpBusPort(self.base, to={"mode": "MASTER"}, sender={"provider": "UNIVERSE"}, thread_id="thr")
        bus.notify(idempotency_key="k1", body_text="done", payload={"role": "HOST", "status": "EXITED", "exit_reason": "MASTER_DONE"})
        bus.notify(idempotency_key="k2", body_text="todo ended", payload={"role": "HOST", "status": "EXITED", "exit_reason": "TODO_DONE"})
        bus.notify(idempotency_key="k3", body_text="idle", payload={"role": "HOST", "status": "EXITED_FAILED", "exit_reason": "IDLE_TIMEOUT"})
        bus.notify(idempotency_key="k4", body_text="worker done", payload={"role": "WORKER", "status": "COMPLETED"})
        self.assertEqual(["INSTRUCTION", "NOTE", "NOTE", "INSTRUCTION"], [call[2]["kind"] for call in Stub.calls])
